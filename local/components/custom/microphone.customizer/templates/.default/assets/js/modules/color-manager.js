...
export function initColorManager() {
    ...
    Object.keys(FREE_VARIANTS).forEach(section => {
        ...
        if (!sectionState || !sectionState.color) {
            ...
            if (firstVariant) {
                const value = firstVariant.ralCode || firstVariant.label.toLowerCase().replace(/\s+/g, '-');
                stateManager.set(`${section}.color`, value);
                stateManager.set(`${section}.colorValue`, firstVariant.hex);
                stateManager.set(`${section}.variant`, firstVariant.isNonRal ? '3' : '1');
            }
        }
    });
}
 ...
 */
export function handleColorSelection(section, value, hex, variant = '1', label = '') {
    // Обновляем состояние через StateManager
    stateManager.set(`${section}.color`, value);
    stateManager.set(`${section}.colorValue`, hex);
    stateManager.set(`${section}.variant`, variant);
    
    // Обновляем цену в зависимости от того, является ли вариант бесплатным
    const isFree = isFreeVariant(section, value);
    const price = isFree ? 0 : 1500; // Предполагаем, что платные варианты стоят 1500₽
    
    // Обновляем цену в StateManager
    const priceKey = section === 'shockmountPins' ? 'shockmountPins' : section;
    stateManager.set(`prices.${priceKey}`, price);
    
    // Обновляем UI
    updateColorDisplay(section, hex, label);
    
    console.log(`[Color Manager] ${section}: ${value} (${hex}), price: ${price}₽, free: ${isFree}`);
}
...
export function createQuickSelect(section) {
    ...
    variants.forEach(variant => {
        const value = variant.ralCode || variant.label.toLowerCase().replace(/\s+/g, '-');
        ...
        div.addEventListener('click', () => {
            handleColorSelection(section, value, variant.hex, variant.isNonRal ? '3' : '1', variant.label);
        });
        
        container.appendChild(div);
    });
    
    console.log(`[Color Manager] Created quick-select for ${section}`);
}
...
export function resetToFreeVariant(section) {
    ...
    if (firstVariant) {
        const value = firstVariant.ralCode || firstVariant.label.toLowerCase().replace(/\s+/g, '-');
        handleColorSelection(section, value, firstVariant.hex, firstVariant.isNonRal ? '3' : '1', firstVariant.label);
    }
}
